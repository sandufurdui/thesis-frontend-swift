...
// MARK: this is to solve the issue that cards are rendered twice
                        var ds = ""
//                                                self.stackView.removeFromSuperview()
    ...
    private var mySection: summarySection {
        let items = itemss
        //        self.mySection = summarySection(items: items)
        //        if items.isEmpty {
        ////            let noItemsMessage = summaryItem(title: "No items found", detailTitle: "")
        //            return summarySection(items: [noItemsMessage])
        //        } else {
    ...
    func configureTable(){
        ...
        NSLayoutConstraint.activate([
            cardView1.topAnchor.constraint(equalTo: stackView.bottomAnchor, constant: 16),
